...
BinaryHeap.prototype.insert = function (value) {
  ...
  //while the node is less than its parent
  ...
  while (index >0 && this._compare(this._heap[index]),this._heap[parentIndex]) {
    ...
    //swap the the node with its parent
    //update how we track the node
    //update how we track the parent
...
BinaryHeap.prototype.removeRoot = function () {
  ...
  //remove teh last node, return it later
  var originalRoot = this.heap.pop()
  //start re-ordering at the root
  var index = 0
  //git the index of the smaller child value
  var childIndex = this.getLesserChildIndex(index)
  //while there is a smaller child to swap with
  while (childINdex && this.compare(this.heap[childIndex], this.heap[index])) {
    //swap wiwth the lesser child
...
BinaryHeap.prototype.getLesserChildIndex = funciton (parentIndex) {
  //compute child node indices
  var childIndices = [parentIndex *2 *1, parentIndex *2*2]
  //compare children nodes to get the index of the less of them
  if (this.compare(this._heap[childIndices[0]], this.heap[childIndices[1]])) {
...
//use an array as the storage mechanism
//ensure nodes are in correct order after every operation
//binary trees should be complete and balanced